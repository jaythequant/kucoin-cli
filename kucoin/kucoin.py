...
class Client(object):
    ...
    def __init__(
        self, api_key="", api_secret="", api_passphrase="", 
    ...
    ## Internal functions used to feeding and processing data
    ...
    ## Heavy duty research functions

    def get_kline_history(self, ticker, begin, end, interval, error_msg=False):
        """
        ticker: Currency pair formatted in upper case (e.g. ETH-BTC)
        begin: datetime input
        end: datetime input
        interval: kline pattern
        Kline Options: 1min, 3min, 5min, 15min, 30min, 1hour, 2hour, 4hour, 6hour, 8hour, 12hour, 1day, 1week
        """
        u_begin = int(calendar.timegm(begin.timetuple()))
        u_end = int(calendar.timegm(end.timetuple()))
        path = f"market/candles?type={interval}&symbol={ticker}&startAt={u_begin}&endAt={u_end}"
        ...
        try:
            ...
            frame[0] = pd.to_datetime(frame[0], unit="s", origin="unix")
            frame = frame.rename(
                columns={
                    0: "time",
                    1: "open",
    ...
    def get_marginable(self):
        ...
        resp = resp.json()
        frame = pd.DataFrame(resp["data"])
        marginTrue = frame["isMarginEnabled"] == True
        tradingEnabled = frame["enableTrading"] == True
        frame = frame[marginTrue & tradingEnabled]
        return frame
